...
> "Built solo, with an AI loop — and **Google Antigravity wrote the very first dev run of this app**. It stayed in the loop after: here it is rendering the UI direction that shipped, and here running a multi-model code review that caught real findings before submission. The eval suite it helped shape does something rare for agent demos: it *attacks* the agent — payment requests, prompt injection in web pages and documents — and pins the safety invariants as tests: no specialist holds a payment tool, every persona refuses to narrate an action that didn't happen."
