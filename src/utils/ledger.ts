...
export function resolveLedgerEntry(
  ...
  if (!refId && !hasChanges) return { status: 'approved', applied: false };
  const targetExists = !!refId && (Array.isArray(events) ? events : []).some(e => e.id === refId);
  if (!targetExists) return { status: 'failed', applied: false };
  if (!hasChanges) return { status: 'approved', applied: false };
