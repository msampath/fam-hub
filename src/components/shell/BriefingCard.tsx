...
export default function BriefingCard() {
  ...
  const { choresList, setShoppingList, authorStamp, goalsList, shoppingList, actionLedger, stageLedgerEntries } = useApp();
  ...
  const loadBriefing = async () => {
    ...
    try {
      const res = await apiFetch('/api/morning-briefing', {
        method: 'POST',
        body: JSON.stringify({ events, chores: choresList, goals: goalsList, shopping: shoppingList, ledger: actionLedger.filter(e => e.status === 'pending') }),
      });
