...
| **Antigravity** | (build/architecture loop) | Wrote the **very first dev run** of the app (before the project moved to Claude Code for the main dev loop), then returned throughout for architecture + multi-model review passes (its UI render shipped nearly as-is); shown in the demo video |
