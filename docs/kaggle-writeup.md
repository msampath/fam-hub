...
2. **Provenance-gated handoffs.** An LLM will happily guess `opentable.com/r/<restaurant>`. Family-Hub's `prepare_handoff` is gated by the MCP server: it stages a booking URL **only if that exact URL was observed in this run's `web_search`/`fetch_page` results** — a link the venue actually publishes. Guessed links are rejected server-side, with the rejection surfaced honestly.
...
4. **Honest capability reporting.** The IoT tool returns `unavailable` (no fake success); every persona forbids narrating an action whose tool call didn't succeed *this turn*; the briefing agent is read-only. When a claim can't be made safely, the agent says so.
