...
Built solo, vibe-coded end to end: **Google Antigravity wrote the very first dev run of this app**, the project then moved to **Claude Code** for the main development loop, and Antigravity returned throughout for architecture and multi-model review passes (its render of the neo-brutalist UI direction shipped nearly as-is) — with Gemini as the runtime brain. The safety posture came from adversarial self-review — a security pass (CSO-style findings, several fixed pre-submission), a FinOps pass that caught a worst-case cost cascade, and an eval suite that tries to make the agent pay for things. The most useful lesson: **the reliable way to make a small model agentic is to move truth out of the model** — pre-fetch the facts, verify the outputs, gate the writes, and let the LLM do the one thing it's uniquely good at: deciding what's worth doing.
