...
Built solo with an AI-assisted loop: **Google Antigravity did the very first dev run of this app**, and stayed in the loop after — driving architecture reviews and multi-model code review passes (its render of the neo-brutalist UI direction shipped nearly as-is), with Gemini as the runtime brain. The safety posture came from adversarial self-review — a security pass (CSO-style findings, several fixed pre-submission), a FinOps pass that caught a worst-case cost cascade, and an eval suite that tries to make the agent pay for things. The most useful lesson: **the reliable way to make a small model agentic is to move truth out of the model** — pre-fetch the facts, verify the outputs, gate the writes, and let the LLM do the one thing it's uniquely good at: deciding what's worth doing.
